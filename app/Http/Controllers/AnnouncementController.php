...
{
    private function sendNotification($userIds, $title, $content, $link = null)
    {
        $notifications = collect($userIds)->map(function ($userId) use ($title, $content, $link) {
            return [
                'user_id' => $userId,
                'title' => $title,
                'content' => $content,
    ...
    public function store(Request $request)
    {
        ...
        $this->sendNotification($validated['target_id'], $validated['title'], Str::limit($validated['content'], 50), $validated['link']);
