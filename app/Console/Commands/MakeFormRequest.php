...
class MakeFormRequest extends Command
{
    ...
    public function handle()
    {
        ...
        if (file_exists($filePath) && !$force) {
            $this->error("FormRequest $className already exists. Use --force to overwrite.");
        ...
        // tulis file FormRequest jika file tidak ada atau force
        if (!file_exists($filePath) || $force) {
            file_put_contents($filePath, $template);
            $this->info("FormRequest $className created successfully at $filePath.");
        } else {
            $this->info("FormRequest $className already exists at $filePath. Use --force to overwrite.");
        }
